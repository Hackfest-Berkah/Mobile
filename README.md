...
## Kiri-FE (Front End)
*Kiri-FE is the front-end repository that contains the source code for the Kiri application's user interface. This Flutter-based front end seamlessly interacts with the Kiri server (Kiri-BE) to provide a user-friendly experience. The Flutter framework, along with Dart programming language, powers the Kiri-FE. This repository focuses on creating an intuitive and visually appealing interface to support users in managing their health effectively. Additionally, the GetX pattern is implemented to manage the state in a predictable way.*
